...
- Avaliação média de performance por subsetor
...
- Dados persistidos automaticamente no `localStorage` do navegador
...
└── src/
    ...
    ├── data.js             → Geração de dados sintéticos (seed inicial)
...
cd Analise-de-Pessoas-TI/people-analytics-vercel
...
O projeto está configurado para deploy automático. Qualquer push na branch principal dispara um novo deploy.

**Primeiro deploy:**
1. Acesse [vercel.com](https://vercel.com) e importe o repositório GitHub
2. Configure o **Root Directory** como `people-analytics-vercel`
...
Os dados são gerados no arquivo `src/data.js` com distribuição realista e seed fixo, garantindo consistência entre sessões. Alterações feitas na aba Gestão são salvas no `localStorage` e persistem entre visitas no mesmo navegador.
